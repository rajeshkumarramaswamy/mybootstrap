...
def layout(request):
    ...
    cached_data_productlines = []
    for pl in lines_records:
        if pl.productline not in cached_data_productlines:
            productlines_data.append(str(pl.productline))
            prod_and_lines.setdefault(str(pl.productline), [])
        else:
            cached_data_productlines = cache.set(productlines_data, productlines_data.append(str(pl.productline)), 60)

        product_records = Products.objects.filter(productline=pl.productline)
    cached_data_products = []
    for pd in product_records:
        if pd.productname not in cached_data_products:
            product_data.append(str(pd.productname.replace(u'\u2019', "'")))
        else:
            cached_data_products = cache.set(product_data, product_data.append(str(pd.productname.replace(u'\u2019', "'"))), 60 )

    return render(request, 'layout.html', {'productlines_data': productlines_data, 'prod_and_lines': prod_and_lines,
                                           'product_data': product_data, 'emp_records_output': emp_records_output,})
